...
Average per order: {total_quantity / total_orders:,.0f} units"""
    ...
    def process_query(self, question, previous_context=""):
        ...
        if agent_context:
            ...
            elif 'revenue' in question_lower and ('generated by' in question_lower or 'rate by' in question_lower):
                # Handle revenue calculation for specific agent (including when "generated" is corrected to "rate")
                working_data = working_data[working_data['status'] != 'Declined'].copy()  # Exclude declined orders
                working_data = self.clean_quantity_data(working_data)
                working_data['revenue'] = working_data['quantity_clean'] * working_data['rate_clean']
                total_revenue = working_data['revenue'].sum()
                return f"Revenue generated by {agent_context}: ${total_revenue:,.2f}"
            else:
                # For revenue queries that don't specify status, still exclude declined orders
        ...
        # Handle revenue calculation for all agents
        if 'revenue generated by all agents' in question_lower or 'revenue by all agents' in question_lower or 'all agents revenue' in question_lower:
            ...
            valid_data = self.clean_quantity_data(valid_data)
            valid_data['revenue'] = valid_data['quantity_clean'] * valid_data['rate_clean']
